...
class Query(graphene.ObjectType):
    ...
    all_planets = graphene.List(
        PlanetType,
        discovery_method=graphene.String(
            description="Filter by discovery method (e.g., 'Transit')"
        ),
        discovery_locale=graphene.String(
            description="Filter by discovery locale (e.g., 'Space')"
        ),
        host_star_type=graphene.Argument(
            SpectralTypeEnum,
            description="Filter by host star's spectral type (e.g., G for Sun-like)",
        ),
        radius_earth_min=graphene.Float(
            description="Minimum planet radius in Earth radii"
        ),
        radius_earth_max=graphene.Float(
            description="Maximum planet radius in Eath radii"
        ),
        mass_earth_min=graphene.Float(
            description="Minimum planet mass in Earth masses"
        ),
        mass_earth_max=graphene.Float(
    ...
    def resolve_all_planets(self, info, **kwargs):
        ...
        if host_star_type := kwargs.get("host_star_type"):
            queryset = queryset.filter(
                host_star__spect_type__startswith=host_star_type.value
            )
